...
namespace K4AdotNet.Sensor
{
    partial class Calibration
    {
        ...
        public sealed class Orbbec : Calibration
        {
            ...
            public static new Orbbec CreateDummy(DepthMode depthMode, ColorResolution colorResolution)
            {
                CalibrationData.CreateDummy(depthMode, colorResolution, true, out var data);
                return new(in data);
            }
            ...
            /// <exception cref="ArgumentOutOfRangeException"><paramref name="depthMode"/> and <paramref name="colorResolution"/> cannot be equal to <c>Off</c> simultaneously.</exception>
            public static new Orbbec CreateDummy(DepthMode depthMode, ColorResolution colorResolution, float distanceBetweenDepthAndColorMm)
            {
                CalibrationData.CreateDummy(depthMode, colorResolution, true, distanceBetweenDepthAndColorMm, out var data);
                return new(in data);
            }
            ...
            /// <exception cref="ArgumentOutOfRangeException"><paramref name="depthMode"/> and <paramref name="colorResolution"/> cannot be equal to <c>Off</c> simultaneously.</exception>
            public static new Orbbec CreateFromRaw(byte[] rawCalibration, DepthMode depthMode, ColorResolution colorResolution)
            {
                ...
                var res = NativeApi.Orbbec.Instance.CalibrationGetFromRaw(rawCalibration, Helpers.Int32ToUIntPtr(rawCalibration.Length), depthMode, colorResolution, out var calibration);
            ...
            /// <summary>Helper method to create <see cref="Transformation"/> object from this calibration data. For details see <see cref="Transformation.Orbbec(in CalibrationData)"/>.</summary>
            /// <returns>Created transformation object. Not <see langword="null"/>.</returns>
            /// <seealso cref="Transformation.Orbbec.Orbbec(in CalibrationData)"/>.
            public override Transformation CreateTransformation()
                => new Transformation.Orbbec(in Data);
        }
    }
}
